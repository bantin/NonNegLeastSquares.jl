...
nnls_ref(A,b) = nonneg_lsq(A,b;alg=:nnls)
nnls_proj_grad(A,b) = nonneg_lsq(A,b; alg=:projected_gradient)
...
# Test that projected_gradient produces the same solution as scipy
x = [0.15512102, 0.69328985] # approx solution from scipy
@test norm(nnls_proj_grad(A,b)-x) < 1e-5
...
@test norm(nnls_proj_grad(A2, b2)-x2) < 1e-5
